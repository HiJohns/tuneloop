...
- 师傅发现与描述不符需加钱：提交**加价申请**（新修理费 + **到此为止修理费**，二者同填）
- 用户选择：
  - **继续** → **立即补差价**（虚拟商品支付）→ 师傅继续修理
  - **不继续** → 师傅停止修理 → 乐器进入**待发回**
- 加价记录留痕（原报价/新报价/用户决定）
...
- ops: `POST /repair-services/:id/complete`（师傅）；`POST /repair-services/:id/dispatch {leg_fee}`（员工）

## RS-08 结算（系统，末段发回时触发）

- 结算口径：**预付（修理费+物流费预估） vs 实际（到此为止修理费 + 各段实际物流费合计）**
  - 实际 < 预付 → **退款**（微信原路 + 系统通知）
