...
> **管理入口（#1897，2026-09-13 决策）**：平台员工**管理**仅**系统管理员**可用（PC 菜单/路由与 API 双重门控）；成员列表以当前操作员 oid（其顶级组织）为过滤条件并按 role（staff/系统管理员）筛选，**不依赖** `PLATFORM_ROOT_ORG_ID`。本节下方的 env 识别仅用于请求态 PlatformStaff 业务角色（未配置时该分支不触发）。
