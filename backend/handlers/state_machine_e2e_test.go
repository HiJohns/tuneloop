...
func TestInspectReturn_Good_ExecutesRefund(t *testing.T) {
	...
	require.NoError(t, db.Create(&models.Order{
		...
		Status:            models.OrderStatusReturning,
		Deposit:           models.FromYuan(500),
		CashPaid:          models.FromYuan(300),
		PrepaidPointsUsed: models.Cents(20000),
		ReturnedAt:        &now,
...
func TestComputeSettlement_CashPaidExcludesDeposit(t *testing.T) {
	...
	order := models.Order{
		...
		CashPaid:          models.FromYuan(6030), // ¥3000 rent + ¥3000 deposit + ¥30 shipping
		PrepaidPointsUsed: 0,
		GiftPointsUsed:    0,
		Deposit:           models.FromYuan(3000),
		ShippingFee:       models.FromYuan(30),
		StartDate:         &start,
		EndDate:           &end,
		ReturnedAt:        &now,
		Status:            "completed",
