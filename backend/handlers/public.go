...
func GetPublicInstrumentByID(c *gin.Context) {
	...
	response["rent_to_own"] = rentToOwn
...
func GetPublicInstrumentPricingV2(c *gin.Context) {
	...
	if err := db.Where("tenant_id = ?", instrument.TenantID).First(&config).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			config.Config = `{"deposit_mode":"ratio","deposit_ratio":0.3,"deposit_multiplier":7,"tiers":[{"days_max":30,"discount_percent":0},{"days_max":365,"discount_percent":20},{"days_max":-1,"discount_percent":40}]}`
		} else {
