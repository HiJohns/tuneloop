...
func (h *SiteMemberHandler) AddMember(c *gin.Context) {
	...
	res, fail := addMembersCore(c, db, site, tenantID, siteID, input)
	if fail != nil {
	...
	responseData := gin.H{
		"site_id":        siteID,
		"directly_added": res.DirectlyAdded,
	}
	if len(res.BindErrors) > 0 {
		responseData["bind_errors"] = res.BindErrors
	}
	if len(res.InitialPasswords) > 0 {
		responseData["initial_passwords"] = res.InitialPasswords
	}
	if len(res.RoleErrors) > 0 {
		responseData["role_errors"] = res.RoleErrors
	}
