...
func TestRegister_GiftPointsCents(t *testing.T) {
	...
	require.NoError(t, db.Create(&models.User{
		...
		Username: "gift-cents", Status: "active",
	}).Error)
	seedPointsBatch(t, db, userID, models.Cents(9900)) // #1983: balance from batch
...
func TestWalletInfo_PromoPointsCents(t *testing.T) {
	...
	require.NoError(t, db.Create(&models.User{
		ID: userID, IAMSub: userID, TenantID: tenantID, OrgID: orgID,
		Username: "wallet-cents", Status: "active",
	}).Error)
	seedPointsBatch(t, db, userID, models.Cents(20000)) // #1983: balance from batch
