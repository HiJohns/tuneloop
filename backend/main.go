...
func setupAPIRoutes(r *gin.Engine, iamService *services.IAMService, permRegistry *services.PermissionRegistry) {
	...
	regSessionHandler := handlers.NewRegistrationSessionHandler(database.GetDB())
	...
	// Public browsing routes (no auth required, but no-cache so browsers
	// never serve stale instrument state — e.g. stock_status flips after
	// order cancellation, #1625).
	publicGroup := api.Group("")
	publicGroup.Use(middleware.NoCache())
	{
		publicGroup.GET("/public/instruments", handlers.GetPublicInstruments)
		publicGroup.GET("/public/instruments/:id", handlers.GetPublicInstrumentByID)
		publicGroup.GET("/public/instruments/:id/pricing-v2", handlers.GetPublicInstrumentPricingV2)
	...
	authRequired.Use(middleware.RequirePasswordNotForceChange())
	{
		...
		membershipAdmin.GET("/admin/membership-levels/:id/benefits", middleware.RequireCusPerm("membership:manage"), handlers.ListLevelBenefits)
		...
		promoAdmin.Use(middleware.RequireCusPerm("promo:manage"))
		promoAdmin.GET("/admin/promo-plans", handlers.ListPromoPlans)
		promoAdmin.POST("/admin/promo-plans", handlers.CreatePromoPlan)
		promoAdmin.PUT("/admin/promo-plans/:id", handlers.UpdatePromoPlan)
		promoAdmin.DELETE("/admin/promo-plans/:id", handlers.DeletePromoPlan)
		promoAdmin.GET("/admin/promo-plans/:id/details", handlers.GetPromoPlanDetails)
		promoAdmin.PUT("/admin/promo-plans/:id/details", handlers.UpdatePromoPlanDetails)
		// Promo plans (merchant)
		merchantPromo := authRequired.Group("")
		merchantPromo.Use(middleware.RequireCusPerm("promo:manage"))
		merchantPromo.GET("/merchant/promo-plans", handlers.ListPromoPlans)
		merchantPromo.POST("/merchant/promo-plans", handlers.CreatePromoPlan)
		merchantPromo.PUT("/merchant/promo-plans/:id", handlers.UpdatePromoPlan)
		merchantPromo.DELETE("/merchant/promo-plans/:id", handlers.DeletePromoPlan)
		merchantPromo.GET("/merchant/promo-plans/:id/details", handlers.GetPromoPlanDetails)
		merchantPromo.PUT("/merchant/promo-plans/:id/details", handlers.UpdatePromoPlanDetails)
		// Instrument promo overrides
		instrumentOverride := authRequired.Group("")
		instrumentOverride.Use(middleware.RequireCusPerm("promo:override"))
		...
		authRequired.PUT("/instruments/:id/status", middleware.RequireDebugMode(), middleware.RequireCusPerm("instrument:update"), handlers.UpdateInstrumentStatus)
		authRequired.PUT("/orders/:id/admin-update", middleware.RequireDebugMode(), handlers.AdminUpdateOrder)
...
func main() {
	...
	// One-off migration: convert legacy banner images to WebP
	if *migrateBannerWebP {
		count, err := handlers.MigrateBannerImagesToWebP(*dryRunFlag)
		if err != nil {
			fmt.Printf("FATAL: Banner WebP migration failed: %v\n", err)
			os.Exit(1)
		}
		if *dryRunFlag {
