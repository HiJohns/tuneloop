...
// #1857: 业务日历日 = 北京时间（docs/spec/api/api.md 约定，与后端 time.Local=Asia/Shanghai
...
const EVENT_LABELS = {
  ...
  // #1856 教训：缺映射会原样输出 deposit_refunding 等英文码）
  reserved: '未支付',
  deposit_refunding: '押金退款中',
  damage_appealing: '定损申诉',
  pending_damage_response: '待回应定损',
...
export default function OrderDetail() {
  ...
  const rentalDays = (effStartDate && (order.end_date || order.returned_at))
   ...
   // #1803 T3: 金额明细已由 fee_detail 应付段展示；此处仅保留日期/天数补充信息。
   const actualReturnedAt = order.returned_at
   const actualDays = order.actual_rent_days ?? 0

   // #1857: 业务日历日=北京（docs/spec/api/api.md 约定，与后端 time.Local=Asia/Shanghai 对齐）。
  ...
  return (
    <View className="h-screen flex flex-col" style={{backgroundColor: '#FDFBF7'}}>
      ...
      {/* Instrument Info */}
      <View className="mx-4">{instrument && <InstrumentInfo instrument={instrument} onClick={() => navigate(`/instrument?id=${instrument.id}`)} />}</View>
      ...
      <View className="bg-white mx-4 mt-3 rounded-2xl shadow-sm p-4">
        ...
        <View style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {order.fee_detail && (
            <>
              ...
              {order.fee_detail.net_block?.direction && order.fee_detail.net_block.direction !== 'none' && (
                <>
                  <Text className="text-xs font-bold text-zinc-400 mt-3">结算差额</Text>
                  <View className="flex justify-between text-sm">
          ...
          {/* ② 实际租期与结束日期（金额明细已由 fee_detail 应付段展示，#1803 T3 去双口径） */}
          {(actualReturnedAt || actualDays > 0) && (
            <>
              <Text className="text-xs font-bold text-zinc-400 mt-2">实际租期与结束日期</Text>
              {actualReturnedAt && (
                <View className="flex justify-between text-sm">
      ...
      {/* Settlement Status (金额明细已由 fee_detail 三段展示，#1803 T3 去双口径；此处仅保留执行状态) */}
      {settlement && (
      <View className="bg-white mx-4 mt-3 rounded-2xl shadow-sm p-4">
