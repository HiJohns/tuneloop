...
| **UI 文案污染** | placeholder/标签文本含 `#数字`（Issue 编号泄漏到 UI） | 物流费占位符含 #1541（#1628） |
| **根容器背景** | 页面根 `min-h-screen` 无 `bg-[#FDFBF7]`/`bg-brand-bg` | 页面白底不符 ui.md（#1627） |
| **空面板风险** | 卡片 div 无条件渲染但内容全被内层条件门控 | 无点数时点数卡片空渲染（#1626） |
| **replace 导航** | `navigate(..., {replace:true})` 关键流程跳转 | 回退键跳过 checkout（#1629） |

## 已知限制
- 控件门控依赖 YAML `controls` 文本与 JSX 源码精确匹配（含 emoji 的控件名需在 YAML 中同步）
- 数据刷新/背景检查依赖 `_page_jsx` 路径映射表（新增页面需补充映射）
- 空面板检测为模式匹配（外层 div + 紧跟条件块），复杂嵌套可能有漏检
