...
**规则**：
1. **动词 + 宾语**（行动导向）：`提交归还` / `提交发货` / `确认支付` / `发起支付` / `支付会员费` / `确认退款`——**禁止裸动词**（『提交』『确认』必须带宾语）
2. **支付/退款类按钮带金额**：`确认支付 ¥6000.00`、`发起支付 ¥0.99`（金额两位小数，`¥xx.xx`）
3. **进行中状态统一**：`处理中...`（所有页面一致，不得混用 提交中/确认中/处理中）
4. **disabled 状态**：按钮灰显（内联 style `opacity` 或灰底）+ **文本说明前置条件**（如 `请先拍照存档` / `请填写物流信息`），让用户知道为什么不能点
5. **成功反馈**：轻操作 toast（`发货成功`）+ 业务跳转；重操作（支付/退款）进对应结果页
6. **审计清单**：diff 出现裸『提交』『确认』按钮、或 `提交中/确认中` 等非 `处理中...` 文案 → REJECT 要求按本规范统一
...
- **⚠️ 迁移工件断言 + 快照库恢复 SOP**（#1986 教训：预生产快照库 `orders.order_no` 在 `schema_migrations` 记录已应用却实际缺列 → 启动 FATAL）：
  - 迁移若**增表/增列**，必须同步在 `backend/database/db.go` 的 `defaultMigrationArtifacts` **登记断言**（版本 → 表/列）；启动 `verifyMigrationArtifacts` 交叉校验「`schema_migrations` 版本 ≥ N 但工件缺失」→ **FATAL 拒绝启动**（fail-fast，早于业务路由）。
  - **快照库恢复/重建后必须**：① 校验 `schema_migrations` 与实表工件一致（或直接依赖启动校验）；② 不一致时按迁移语义**重放缺失工件**（等价 DDL）；③ **禁止只改版本号**（会造成记录 ≠ 实表）。
...
> 来源：#1771 — human/AI 审计反复遗漏同类缺陷，编译器是唯一不疲劳、不遗漏的审核者。

### 核心原理

将审计规则编码到**类型系统或编译器**中，让违反规则的代码无法编译通过——比人工/AI 审计更可靠。

### 审计清单（强制）

| 审计项 | 方法 | 来源 |
|--------|------|------|
| **废弃模型删除** | 删除 struct 后 `go build .` 必须通过（编译器发现所有引用点） | #1771 M1 |
| **Schema 列校验** | `validateModelColumns` 启动时校验每个模型的所有列是否存在；缺列 = FATAL 拒绝启动 | #1716/#1727 |
| **`Cents` 字段禁止裸字面量** | struct 赋值必须用 `models.FromYuan(n)` 或 `models.ToCentsPtr()`，禁止 `Cents(10000)` 或 `DepositDeducted: 10000` | #1757 |
| **`models.Cents` 别名审计** | 禁止 `type Cents = int64`（别名无编译期保护）；必须 `type Cents int64`（独立类型） | #1757 |

### 典型反例

```go
// ❌ 编译通过但语义错误：裸字面量无类型保护
DepositDeducted: 10000,  // 10000 分？10000 元？编译器不检查

// ✅ 强制使用构造函数：语义明确
DepositDeducted: models.FromYuan(100),  // 100 元 = 10000 分
```
...
6. **为其他仓库创建的 Issue 本身必须打上 `status:todo` 标签**（如 `gh issue create --repo HiJohns/beaconiam --label "status:todo" ...`）
...
**发现实现缺失时→输出关键结论，等待用户确认才建议修改代码。**

#### Step 3.5: 入口可达性 BFS 审计（Reachability Audit）

> 来源：#1589 H5 个人中心编辑资料——weapp 有 `✏️ 编辑资料` 菜单行，H5 只有头像 `onClick`（无文字标签），用户找不到入口。

**目的**：静态确认每个功能页（从 cases.md 功能列表中提取）存在一条**人可识别的路径**可从首页通过界面点击到达。

**方法**——从首页根节点出发逐跳做静态 BFS：

```
1. 首页 JSX 提取所有 onClick / navigate / redirectTo / switchTab / Taro.navigateTo
2. 将每个跳转目标加入队列递归（去重，最大深度 ≤5）
3. 对每个功能页判定至少有一条路径经过【文字标签】或【图标】：
   - 有效标签：<Text>"xxx"</Text>、<Button>xxx</Button>、CSS ::after content
   - 无效标签：头像 onClick（裸图片无文案）、无任何外层标签的 onClick
4. 跨端对等性：H5 与 weapp 进入同一功能的入口是否存在且位置一致
```

**模板**（替代 "Key Findings" 中的实现缺失表述）：

```
| 功能页 | weapp 路径 | H5 路径 | 判定 |
|--------|-----------|---------|:---:|
| 编辑资料 | Profile→"✏️ 编辑资料"→EditProfile | Profile→头像onClick→Modal | ❌ H5 无文字入口 |
```

**判定规则**：
- 两端口径不一致且一端无文字入口 → **设计缺失**（ui.md 未规定入口样式）
- 两端口径一致 → 正常
- 功能页不可达（无路径）→ **实现缺失**
...
### 📋 列表页纵深审计 (List Page Vertical Audit)

> 来源：#1545 用户管理列表演练。适用于包含数据列表展示的页面。

对每个列表字段逐一检查以下三个维度：

#### 维度 1：数据来源

**API → DB 表/列映射**：列表字段从哪个 API 的哪个字段读取？最终数据来源是哪张表哪个列？有无 JOIN 或中间处理？

```bash
# 提取后端所有返回字段
grep -n 'gin.H{\|"json:' handlers/user_management.go | grep -oP '"\w+"' | sort -u
# 对比前端期望的 dataIndex
grep -n "dataIndex" frontend-pc/src/pages/System/UserManagement.jsx
```

**特别注意**：字段名与实际语义不一致的（如 `last_active` = `UpdatedAt` 而非实际活跃时间）。

#### 维度 2：前后端契约

| 方向 | 检查 |
|------|------|
| **上行**（前端→后端） | 请求参数名 vs `c.Query()`/`c.DefaultQuery()` 名称是否一致；分页参数约定（`pageSize` vs `page_size`）是否统一 |
| **下行**（后端→前端） | 响应 JSON key 与前端的 `dataIndex`/`resp.data.xxx` 访问路径是否一致 |
| **写入**（前端→DB） | 前端发送字段名 vs 后端 `ShouldBindJSON` json tag vs DB 列名三者是否对齐 |

**参考**：AGENTS.md §《前后端集成审核清单》维度 1-3。

#### 维度 3：空值安全

- 前端对每个 `dataIndex` 字段是否有 `null`/`undefined` 防御（如 `v ? v : '-'`、`v \|\| '-'`）
- 后端返回的可能为空的字段：`*int`、`*string`、可选 JOIN 结果
- 特别关注状态枚举字段——后端可能返回非预期值（旧数据/未初始化），前端应有 fallback 渲染

#### 维度 4：性能

- 列表查询是否有 N+1 问题（每条记录触发一次独立 SQL）
- 导出是否全量扫描
- 分页/搜索是否走索引
