...
| 待审核 | 最新批次 `pending` | 自拍素材（图/视频）+ 提交/审核时间 | 「确认通过」（填 5 项实名信息）/「驳回」（必填原因）/「唤出证件照对比」 |
| 已通过 | 最新批次 `approved` | ✓ 已核身 + 核身时间 + 通道（人工/自动） | 无 |
| 已驳回 | 最新批次 `rejected` | 驳回原因 + 提交时间 | 无（顾客端重新采集） |

- 数据源：`GET /admin/face-review/user/:userId`（该用户全部批次 + 自拍素材 URL，含历史驳回/通过）
- 通过/驳回：复用 `POST /admin/face-review/:batchId`（与独立审核队列页同 API、同权限 `SysPermUserUpdate`）
- 通道：`face_verify_method=manual` 显示审核操作；`tencent`（自动识别）仅展示反馈结果不显示审核按钮
- **证件照对比**：点击弹出证件照三张（与自拍并排核对）
...
- 费用信息（#1800 单一口径三段，后端 fee_detail 服务端计算，前端只读渲染；金额全部分）：
  - **实付段**：合同租金（含下单日期）→ 各阶梯明细（`第N阶梯 ¥x/天 × D天 = ¥y`，缩进）→ 押金 → 各次续费（含阶梯明细；历史记录无天数时仅金额）→ 合计实付
  - **应付段**：实际租金（含阶梯明细）→ 逾期费（含天数）→ 物流费 → 实际应付
  - **净额段**：应退款 / 应补缴（无差额时不显示）
