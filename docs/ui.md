...
- 基础信息
  ...
  - 分类、级别
- 租金信息
  - 日租金（instrument.pricing.daily_rent）
...
### 3.28 移动端首次登录引导页 (`/onboarding`) — **已废弃**
...
### 3.41 维修服务页组（weapp + PC 后台）（#1942，**2026-09-18 设计变更：师傅直属商户**）

> 完整流程见 `docs/cases/repair-service.md`（RS-01~RS-13）。页面组：
> - **维修 Tab 首屏 = 师傅列表**（样式参照乐器列表）：个人照片 + 姓名 + 详细介绍摘要（如「钢琴维修 12 年 · 小提琴维修 8 年」）；**右上角『我的维修』**——无活跃会话置灰（可点）、有活跃会话点亮+个数 → 进入「我的维修」（现存维修页=师傅工作台，看自己的会话）
> - **师傅详情页**（样式参照乐器详情页）：照片 + 完整介绍（专长/年限）；下方固定「**创建维修订单**」→ 创建页（**师傅锁定，无需选商户/网点**）
> - 用户（weapp）：创建维修单（描述/照片，提交后展示 **6 位唯一编码**）| 报价接受/支付（虚拟商品）| 寄出填单 | 加价响应（继续补差价/停止）| 评价（评分/留言/拍照）
> - 师傅（weapp，**直属商户**）：报价（修理费+物流费预估，受控模式预估 3 段）| 加价（新总价+到此为止修理费）| 完成修理
> - 网点员工（weapp）：分段发运（实填本段物流费）| 待发回清单 | 发回（末段实填 → 触发结算）
> - PC 后台：维修服务全量列表 + 评价（评分/留言/照片）展示；**师傅档案维护**（新增/编辑/停用：照片/介绍/专长年限）
> - **师傅归属**：直属商户（tenant），不再挂靠网点（RS-13）

### 3.42 乐器丢失与恢复入口（员工 weapp + PC 后台）（instrument-loss.md）
