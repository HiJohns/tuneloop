...
**订单动态时间线**（#1848）:
- 归还页与订单详情页时间线格式一致（共享 `components/OrderTimeline.jsx`）：每条日志正文一行 + 「MM-DD HH:mm · 操作人」（system 显示为「系统」）一行；事件文案映射与详情页 EVENT_LABELS 同表
...
6. **会员手册卡**（#1830 新增，页面最底部）：标题「会员手册」+ 展开/收起（收起为一行标题）；正文为**统一静态文案**（所有会员一致，`HANDBOOK_SECTIONS` 常量：会员体系 / 升级门槛 / 返现积分 / 积分使用 / 其他权益），参数表述以当期政策为准
