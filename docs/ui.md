...
> 创建和编辑都统一为面板替换模式：点击按钮 → 列表面板**替换**为表单 → 提交成功自动翻转回列表。

**组件**: `frontend-pc/src/pages/StaffManagement.jsx`  
**位置**: 人员管理页中，使用 `viewMode` 控制（'list' | 'create' | 'edit'）
...
├─────────────────────────────────────────────────┤
│  点击行「编辑」→ viewMode='edit'                  │
│  ───────────────────────                         │
│  编辑表单（替换列表面板）：                        │
│   姓名、邮箱（改邮箱需确认）、手机                │
│   归属网点、角色                                  │
│   [保存] [取消] → setViewMode('list')            │
└─────────────────────────────────────────────────┘
```
