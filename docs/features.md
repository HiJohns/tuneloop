...
### 3. 订单与支付
* **免押金体系**：支持会员等级或信用分免押金模式（模拟/接入）。**申请条件收紧（#1867）**：仅限已实名核验的学生／教职工（第三证件类型 `student`/`teacher`，#1807），内部信用分达到门槛（`DEPOSIT_WAIVER_MIN_CREDIT`，默认 600），须提供 2 位担保人信息，并上传签名的推荐信（可下载模板，员工下单后审核，不符可取消订单）。
