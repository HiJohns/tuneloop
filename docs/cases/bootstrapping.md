...
- IAM 是权威源，本地仅缓存（#685）
- 操作顺序：先 IAM 后本地
- **既有账户（#2025 D2 裁定·细化）**：被绑定人手机号/邮箱已有账户 → **不允许管理员挂接**（一人一号）；返回 40902 指引本人自助加入（作为员工加入 / 注册为顾客）
- **#2052 通知邀请（取代 409 硬错 / 邀请码）**：创建员工时撞库 → 系统向既有账户发送**加入邀请通知**（`type=staff_invite`，不发码）→ 被邀请人在「系统消息」**接受**（复用绑定：IAM `BindUserToOrganization(本人 IAMSub, org, role)` + 本地 `site_members`）或**拒绝** → 接受时校验「处理人 = `invitee_user_id`」防顶替；幂等
