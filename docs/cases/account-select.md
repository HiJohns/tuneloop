...
- 「注册为顾客」链接（无顾客上下文时显示）→ 注册页（加入顾客组织 relation；员工身份不变）
...
- **身份模型（#2025）**：注册即加入**顾客组织 relation**（一人一记录）；手机号已注册的添加成员按 D2 **管理员信任制直接复用**（无 SMS 验证）；被标记删除用户再登录按 D3 重新激活原户；解绑按 D4/#2020 删除 `wx_user_bindings` 行
