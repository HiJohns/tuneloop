...
## 账户列表页（/account-select，组织上下文）

> **#2025/#2026 身份模型**：一人一记录 + 多组织上下文。列表不再展示「账户昵称（多个账户）」，而是**组织上下文**：`组织名 + 角色标签`（如「顾客」/「新街口琴行 · 新街口店」）。顶部 greeting「欢迎 {name}」。详见 `docs/topics/iam/iam-notes.md` 身份模型章节。

- 顾客上下文 → 标签「顾客」
- 员工上下文 → `{商户名 · 网点名}`（主组织）
- 「用户名密码登录」链接（仅员工场景显示，判定口径 = **关联账户存在非顾客上下文**；#2077：不可用 `!is_customer`——B1 后自服务注册用户一律带 `customer` 角色，「顾客+员工」双身份账号会被误隐藏）→ 展开账号密码面板
- 「注册为顾客」链接（无顾客上下文时显示）→ 注册页（授予 `customer` 零权限角色；员工身份不变）
- 点击上下文 → `POST /auth/wx-login-select { code, context }`（组织上下文按其 relation、**顾客上下文按 `customer` 角色专用签发**；顾客上下文 tid 保持空，D1）→ 登录 → 回目标页
- 同一微信号的历史多条绑定（存量多户，兼容期）→ 双形态兼容展示，待 #2029 合并
...
- 拥有 ≥2 组织上下文（如顾客 + 员工）→ 「退出登录」+「切换账户」→ 账户列表页（组织上下文选择）
- **未登录（guest）** → 登录按钮三态：
...
- 账户列表页：组织上下文（顾客标签 `顾客`；员工 `商户名·网点名`），欢迎语「欢迎 {name}」
- 注册页：无用户名、昵称可编辑、mode=member 隐藏用户名密码登录、提交按钮「支付会员费」
- **身份模型（#2025/#2027 B1）**：注册即授予 **`customer` 顾客角色**（零权限，附着顶层组织 member 关系；一人一记录）；手机号已注册的添加成员按 D2 **管理员信任制直接复用**（无 SMS 验证）；被标记删除用户再登录按 D3 重新激活原户；解绑按 D4/#2020 删除 `wx_user_bindings` 行
