...
> 版本: v1.0
> 最后更新: 2026-09-17
> 来源: cases.md 迁移拆分（#1583）；顶层编号清洗见 cases/README.md
...
> **已迁移**：维修域完整内容已迁入 `cases/repair.md`（R-01 客户报修 v3 / R-02 报价调整）、`cases/lease-repair.md`（R-03 租赁乐器维修）、`domains/repair.md`（域索引：角色权限 R1-R7、数据模型、费用模型、页面路由、API 汇总）。本节保留索引，正文不再重复。
>
> - 维修状态机（§3.1）、维修流程 R1-R5 规则 → `domains/repair.md` §2 + `cases/lease-repair.md` R-03
> - 客户报修 v3（§3.4，全权/合作路径、状态机、角色视角、费用模型）→ `cases/repair.md` R-01 + `domains/repair.md` §4-5
> - 维修师傅管理（§3.2，⚠️ 已废弃 #1888 R6）→ `domains/repair.md` §2
...
- 如有冲突，弹出选择对话框列出已有用户
- 可选择"继续创建"或"选择已有用户"
- **手机号已注册（#2025 D2 裁定·细化）**：**不允许管理员代为挂接**（一人一号，防误绑）；返回 40902「该手机号/邮箱已在本平台注册，请让本人登录后自助加入（作为员工加入 / 注册为顾客）」（当前无 SMS provider，不引入短信验证）
创建成功，对话框关闭，列表刷新
...
> v3 与旧版差异：**报价前置且多受控网点竞价**（非中转网点单选转发）；**单一报修单+双向脱敏**（非另建"真实报修单"）；实物运输在付款后。详见 `docs/domains/repair.md` §4-5。
