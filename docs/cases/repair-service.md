...
| 链路 | 创建方 | 载体 | 入口 |
|------|--------|------|------|
| 顾客「维修服务单」 | 顾客（选维修师） | `repair_requests.type='service'` + `technician_id` | `/tech-list` → `/tech-detail` → `/repair-service-create?technician_id=` |
...
| 计费 | 定损金额 | **维修师报价（修理费+物流费）+ 加价** |
| 物流 | 网点/中转安排 | 用户自寄出（编码标记）+ 网点员工发回（**分段物流费**） |
| 评价 | 无 | 评分/留言/拍照，PC 后台可见 |

## RS-01 创建维修单（用户，weapp）——**入口经维修师详情**（2026-09-18 设计变更）

- **入口链路**：「维修」Tab → **维修师列表**（RS-02）→ 点击维修师 → **维修师详情页**（RS-02）→ 下方「**创建维修订单**」按钮 → **维修单创建页**
- **创建页要点**：**维修师锁定 = 刚才查看的维修师**（只读展示维修师头像/姓名）；**无需选择商户与网点**
- 表单：描述（必填）+ 照片（≤6）+ **视频（可选，≤1 段，试奏，#2060）**——**不填识别码**
- **视频（试奏）规则（#2060，2026-09-24 用例先行）**：
  - **可选**（描述+照片已必填；视频为增强信息）；最多 **1 段**，复用 `repair_requests.video_url`（varchar(500)，legacy 列，零迁移）
  - 用途：试奏/故障复现录屏，供维修师接单前判断故障（比照片直观）
  - 上传：`POST /api/upload`（现有视频分支：mp4/webm/mov，大小受 `video_max_size` 系统设置约束）→ 取返回 `file_key` 填入 `video`
  - 双端选片：weapp `Taro.chooseMedia({mediaType:['video']})`（可设 `maxDuration`）；H5 `<input type="file" accept="video/*">`；已传可预览/删除（替换）
  - 展示：维修单详情（用户 RS-12）与维修师工作台均可**播放**（weapp `<video>` 组件）；无视频时不渲染播放区
  - 存储：沿用统一媒体管线（当前 LocalStorage；如启用 `STORAGE_MODE=dual/oss` 则视频同样进 OSS，无需改用例）
- 提交后系统分配 **6 位唯一编码**（数字+大写字母，`repair_code` uniqueIndex，冲突重试），界面展示并提示「请将该编码写在物流单信息栏」
- **调用 / 获取**
  - `POST /api/user/repair-services {description, photos[], video?, technician_id}`（**创建即锁定维修师**）→ `{id, repair_code, status}`
  - 照片上传：`POST /api/upload`（现有）→ 取返回 url 填入 `photos`
  - 服务单归属 = **维修师直属商户**（`tenant_id` 回填；**不再有 site 维度**，见 RS-13）
- **兼容**：`POST …/select-technician` 保留（老数据/未锁定单补选），新流程创建时即锁定

## RS-02 维修师列表 / 维修师详情（用户浏览）＋ 报价（维修师，weapp 工作台）

### 维修师列表（维修 Tab 首屏，2026-09-18 变更）
...
- **下方固定「创建维修订单」按钮** → 进入创建页（RS-01，维修师锁定）

### 维修师档案与归属（2026-09-18 设计变更，见 RS-13）
- **维修师不再挂靠网点**，**直属商户**（tenant 级）；`tenant_id` 即服务单归属
- 档案字段：`photo`（个人照片）、`bio`（详细介绍）、`experience`（专长与年限，结构化：`[{craft, years}]`）

### 报价（维修师，weapp 工作台）
- 维修师**报价**：`修理费` + `物流费预估`
  - 直连模式（无中转）：1 段受管物流（**维修师→用户**）
...
- 受控组合的段序：中转→受控（网点员工）→（维修师修理）→ 受控→中转（受控侧员工）→ 中转→用户（网点员工，**末段触发结算**）
...
## RS-06 加价（维修师报价修正，weapp 工作台）

- 维修师发现与描述不符需加钱：提交**加价申请**（新修理费**总价** + **到此为止修理费**，二者同填且语义不同）
  - 例：报价 200（4h×50/h）+ 物流预估 50 → 用户已付 250；干了 1h（到此为止 = 50）后发现需加物料 100 → 新总价 = 300，到此为止 = 50
- 用户选择：
  - **继续** → **立即补差价** = `新总价 − 原报价修理费`（= 100；**物流费不参与补差**，按各段实填结算）→ 虚拟商品支付 → 维修师继续修理
  - **不继续** → 维修师停止修理 → 乐器进入**待发回**（结算修理费基准 = **到此为止修理费**）
- 加价记录留痕（原报价/新报价/用户决定）
- **调用 / 获取**
  - 维修师：`POST /api/repair-services/:id/adjust {new_quote_cents, incurred_cents}` → `{id, status, payable_cents(差价), incurred_cents}`
...
## RS-07 完成修理（维修师，weapp 工作台）

- 维修师点「**完成修理**」→ 乐器进入**待发回**状态
- **网点员工接管**：联系物流、填单（填**本段实际物流费**）→ 发回 → **触发结算**
- ops: `POST /repair-services/:id/complete`（维修师）；`POST /repair-services/:id/dispatch {tracking_company, tracking_number, logistics_fee_cents}`（员工，末段实填 + 触发结算）
...
| 3 | `pending_quote` | 报价（修理费+物流预估） | 维修师/员工 | **必须已选师**且 JWT 归属匹配；写 `quote_status=pending` | `pending_payment` |
...
> - **员工 / 维修师**：JWT **有** `tid`/`oid` → 端点注册在 **`authRequired`**（`IAMInterceptor`），
...
| 角色 | 能力 |
|------|------|
| 用户（weapp） | 浏览维修师列表/详情 → 创建（维修师锁定）/寄出/支付/加价响应/评价 |
| 维修师（weapp，**直属商户**） | 报价 / 加价 / 完成修理；**『我的维修』**（活跃会话计数） |
| 网点员工（weapp + PC） | 分段发运/实填运费 / 触发结算（物流段仍由网点/中转执行） |
| 平台管理员（PC） | 全量查看 + 评价审核视角 |

## RS-13 维修师档案与归属（2026-09-18 设计变更）

- **归属**：维修师**直属商户**（tenant 级），**不再挂靠网点**。服务单 `tenant_id` = 维修师所属商户；**无 `site_id` 维度**（原「选师后回填网点」作废）
  - 过渡：历史数据中 `technician_id` 来自 `site_members(role=repair_technician)` 的服务单保持原样（不迁移）；新流程以维修师档案为准
