...
> **#2049 师傅档案字段**：`technician_profiles` = 照片（媒体管线：原图 `technician_{id}.webp` + 缩略图 `technician_{id}_thumb.jpg`）+ 富文本简介 `bio`（HTML，移动端 `RichContent` 渲染）+ 专长/年限 `experience`。存量**不迁移**，支持 PC 编辑补录；无照片 → 人像占位符。
