...
> 收货地址归属**个人资料**（不再位于会员中心）。组件：`components/AddressManager.jsx`。

- **入口**：个人资料编辑页（`/profile/edit`）底部「收货地址」区块
- **展示**：地址列表（收货人/电话/省市区详址/默认标记）；空态提示
- **操作**：新增 / 编辑 / 删除 / 设默认（走 `addressesApi`：list/create/update/delete/setDefault）
- **校验**：收货人、电话必填；邮编可选且须 6 位数字
- 会员中心**不含**收货地址
