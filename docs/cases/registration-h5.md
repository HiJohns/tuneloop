...
- **注册即授予顾客身份（B1）**：建户成功后，向用户**根组织** `member` 关系的 `functional_roles` 追加 `customer`（零权限、幂等）；这是顾客身份的**唯一**标记，会员费/等级是叠加其上的产品状态
- **一人一记录**：手机号已注册时不再另建账户（详见 D2 管理员信任制 / 「添加成员复用」）；同一人可由任一既有入口（顾客/员工）复用同一 `users` 记录
