...
- 仅接受 JPEG、PNG、WebP
- 单张 ≤ 5MB
- **前置规则（#2037/R1）**：`other`（第二证件）的提交以 `front`/`back` **均已上传**为前提；未提供身份证照时**不得提交第二证件**（后端拒绝 + 前端引导）
- **一致性规则（#2037/R2）**：第二证件照须与身份证照**对照一致**——列为员工审核（实名队列 / 第二证件复审）的**评审项**；自动校验暂缺（长期目标，本期人工）
- **介绍信口径（#2037/R3，实现于 #2057）**：介绍信**仅在第二证件=学生证时必需**（教职工证/教师证/工作证/其他不需要）
  - 类型前移：用户在提交第二证件时**自报类型**（`second_doc_type`，写入 `users.id_photo_other_type`），审核端保留修正权（指定 student 覆盖）
  - 提交校验：`student` 且未传介绍信 → `40902` + `data.reasons=["no_intro_letter"]`（后端权威，前后端双重拦截）
  - 审核校验：员工指定 `student` 而用户无介绍信 → `40912`（应驳回要求补传）
  - 存储：介绍信经 `POST /api/upload`（content_image）取得存储键，落 `users.intro_letter_url`；审核队列返回可访问 URL
- front/back 可独立上传（须同时具备才允许 other）
