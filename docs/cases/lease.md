...
# L-00C 下单前置条件与待提交订单（#2037 / #2040 / #2041）

## 前置条件（下单必须满足）
- `users.id_photo_front` 与 `id_photo_back` **已上传**
- `users.face_verified = true`（**员工审核放行**后才可下单）

## 不满足时的交互（缓存订单）
- 后端返回专用错误码 **40310 `need_identity`**（`reasons: ["no_id_photo"|"not_verified"]`）
- 前端：提示原因 → **将当前订单写入 `pending_orders`（可多条）** → 跳转 `/profile/edit` 完成证件/实名
- 实名通过通知（`id_verify`）点击 → **直达「待提交订单」列表** → 「继续提交」回跳结算页（`?resume_pending=<id>`）回填续提
- 续提前**校验乐器仍可租**：已被他人租出（`stock_status !== 'available'`）→ 提示「暂存订单已失效」并清理该条
- 已实名用户直接「继续提交」；未实名用户按钮为「完成实名后提交」（引导 `/profile/edit`，暂存保留）
- 提交成功后清理该 pending（标记 `submitted`）
- 免押订单：先过本前置，再过免押资格（40301），两码并存

## 验收
- 未实名/缺证件照 → 40310 且订单进入待提交列表；实名通过后可一键继续提交成功
- 已实名用户下单不受影响
