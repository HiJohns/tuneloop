...
>
> **身份模型基线（#2025 / #2026 S0 裁定，全部用例适用）**：一人一记录 + 顾客组织 + 多组织 relation + 组织上下文切换。
> 四个裁定：**D1** 顾客上下文 JWT `tid` 保持空（隔离体系零改动）｜**D2** 手机号已注册的添加成员走验证码确认复用｜**D3** 被标记删除用户再登录激活原户｜**D4** 删除 = 标记删除 + 解除关联（记录保留、可加回）。
