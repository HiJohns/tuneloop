...
* **实名核身与证件审核**：注册必交身份证正/反面 + 自拍/视频核身批次；第二证件照可选（注册时或注册后在「编辑资料」补交），**类型由平台员工审核时指定**；补交/更换第二证件会重置认证态并**重新进入审核队列**（`second_doc` 批次，#1924/#1921）。
...
### 3. 订单与支付
* **免押金体系**：支持会员等级或信用分免押金模式（模拟/接入）。**申请条件收紧（#1867）**：仅限已实名核验的学生／教职工（第三证件类型 `student`/`teacher`，#1807，且该第二证件须审核认证完成 `id_photo_other_verified`，#1925——仅顾客自填类型不可免押），内部信用分达到门槛（`DEPOSIT_WAIVER_MIN_CREDIT`，默认 600），须提供 2 位担保人信息，并上传签名的推荐信（可下载模板，员工下单后审核，不符可取消订单）。
* **首期汇总**：清晰展示“首月租金 + 押金”的总额。
* **微信支付集成**：支持微信支付（JSAPI 小程序内 / H5 移动端 / Native PC 扫码）。运行时一律真实微信支付（#1719 移除模拟支付）。支付页支持优惠码：OREZ 全免 / ENO 折至 1%（所有支付类型通用，金额后端重算）。
...
* **用户隔离**：
  * 游客购物车：key `cart`
  * 登录用户购物车：key `cart_{user_id}`
  * 不同账户购物车互不影响
* **游客合并**：登录后若游客购物车与用户购物车均非空 → 弹窗询问「是否将之前的乐器合并到当前购物车？」
  * 确认 → 按 `instrument_id` 去重合并，清空游客购物车
  * 取消 → 保持用户购物车，清空游客购物车
* **阶梯计价**：购物车从 `pricing_v2.tiers` 读取阶梯配置，按租期天数分段计算租金
...
* **阶梯房租**：默认定价策略 —— 前 30 天基准价、31–180 天 95 折、180 天后 9 折；逾期按 1.5× 日租金计费（可在 promo_plan_details.overdue_discount 中配置）。
* **押金规则**：押金优先级为用户手动录入 > 总价 × 押金比例（默认 30%）> 日租金 × 押金倍率（默认 7×）。商户管理员可在定价策略中配置比例和倍率。网点管理员可手动覆盖押金金额。
* **维保服务包**：定义各级别乐器包含的免费维保内容（如大师级含专家精调）。

### 3. 支付管理
* **支付方式**：支持微信支付（JSAPI 小程序内 / H5 移动端 / Native PC 扫码），一律真实支付；支持优惠码（OREZ 全免 / ENO 1%，服务端重算）。**金额单位为分**（#1728，1 元 = 100 分）。
* **支付配置**：商户号、APIv3 密钥、证书序列号、私钥路径通过 `.env` 配置。回调地址需在微信商户平台设置白名单。
* **对账能力**：PC 端 `/admin/payments` 页面按时间/类别/方式/状态筛选支付记录，支持 CSV 导出，与微信商户平台对账单交叉比对。
* **退款管理**：押金退款和结算退款通过微信退款 API 原路退回，支持部分退款。退款失败触发站内通知 + 邮件告警。
...
**核心参数**（手册口径 v2，#1939；数值均为可调整配置，禁止硬编码）：
- `pay_ratio`：赠点使用比例（初次付款/续费时抵扣上限，手册默认全额 100%）
- `referral_ratio`：裂变奖励比例（乐手 2% / 首席 5% / 演奏家 8%，**待立项**）

> **退款返点（`refund_ratio`）已废除**：M-06 决策取消「退款返点给自己」，旧表 `membership_gift_ratios` 退役；`points_policies.max_pay_ratio` 并入 `gift_policies.pay_ratio`。
...
租金计算由 `backend/services/rent_calculator.go` 和 `services/pricing.go`（v3 阶梯累加引擎）统一处理。

### v3 按天分段累加算法

租金不再采用单一折扣率×总天数，而是**按阶梯分段累加**：

```
总租金 = Σ(阶梯i的日费率 × 该阶梯覆盖的天数)
```

**示例**：租 70 天，日租金 ¥100，默认阶梯 [1-30 天 0% of, 31-180 天 5% of, 181+ 10% of]：
- 第 1 阶（1-30 天）：¥100 × 30 = ¥3,000
- 第 2 阶（31-70 天）：¥95 × 40 = ¥3,800
- **总计**：¥6,800

### 策略优先级

1. **商户自定义策略**（`MerchantPricingConfig`）— 优先采用
2. **系统默认策略**（`PricingTemplate` + `is_system_default=true`）— 无商户策略时使用
...
**订单完成结算**：订单进入 `completed` 状态时（good 验收自动、damaged 接受/申诉后员工点退款、损坏赔偿支付回调），执行差额结算：按调整后应付 R1 与用户当前级别赠点策略重算赠点上限 A1，超 A1 的赠点退回 promo_points，剩余现金（C0−C1）走微信原路退款；关单后 `total_spending += C1`（实付现金口径），并按 C1 × refund_ratio 发放返点赠点（A2），最后发送完成通知（标准收据 + 感谢 + 赠点到账 + 会员中心链接）。`DepositRefundScheduler` 仅作 `deposit_refunding` 超时兜底。详见 `docs/cases/cases.md §2.7` 与 `docs/cases/lease.md L-06`。
