...
#### 2.6.4a 个人中心「切换身份」按钮（weapp，头部，与「退出登录」并列）

| 可见条件 | 文案 | 跳转 |
|---------|------|------|
| 当前上下文为员工（`claims.role !== 'USER'`）**或** 账号存在多个可登录上下文（#2081） | 切换身份 | `/pages-weapp/account-select/index` |

> #2081：一人多角账号（顾客+员工）以顾客上下文登录时此前无切换入口。多身份账号登录必经身份选择页（`/account-select`）选上下文，该页登录成功时将上下文列表落盘 `login_contexts`；个人中心据此（>1）显示入口。`login_contexts` 在退出登录 / 会话失效降级时清除。
...
### 2.24 移动端账户切换页（组织上下文）（#2025 / #2026）

> **一人一记录 + 多组织上下文**（`docs/topics/iam/iam-notes.md`）。页面 `/account-select`。

- **顶部 greeting**：「欢迎 {name}」（`user.name`，不再展示「账户昵称」多账户语义）
- **上下文列表项**：`组织名 + 角色标签`
  - 顾客上下文 → 标签「顾客」
  - 员工上下文 → `{商户名} · {网点名}`（主组织）
  - 维修师傅 → 标签「维修师傅」
- **入口**：会员中心「切换账户」（仅 ≥2 上下文时显示）
- **操作**：点击项 → `POST /auth/wx-login-select { code, context }`；顾客上下文 tid 空（D1）
- **「注册为顾客」链接**（无顾客上下文时显示）：授予 `customer` 职能角色（B1，零权限），员工身份不变
- **兼容期**：存量多户（同 openid 多 user）双形态展示，待 #2029 合并
...
> 注册页（H5/weapp）身份证照片与第二证件上传区下方展示一行说明：「已上传的证件照将由平台员工人工审核（约 1-2 个工作日），审核结果会通过系统消息通知您」——内联 style，灰字小号。

### 2.23.1 个人资料 · 收货地址管理（#2022）

> 位置：个人资料编辑页（`/profile/edit`，`AddressManager` 组件）。会员中心**不再**包含地址区块。

- 区块标题「收货地址」+「+ 新地址」按钮
- 列表项：收货人 / 电话 / 默认标签；省市区详细地址
- 操作：编辑 / 设默认（非默认时）/ 删除
- 表单：收货人、电话、省市区（weapp 用 `Picker`，H5 用原生 select）、详细地址、邮编（6 位校验）
- 跨端：Taro 组件 + 平台 `dialog`/`getInputValue`

### 2.20.1 PC 乐器管理页（乐器列表 × 库存监控 合并，#2043）

> 菜单合并为一项「**乐器管理**」；`/instruments/list` 与 `/site/stock` 为**入口别名**（同页），**透传 `?status=`**（Dashboard 链接不得破坏）。
...
> 成员管理（网点/中转）角色列为 **多选**（`Select mode="multiple"`）：展示并直接编辑一个成员在该网点的**全部角色**（如 员工 + 维修师傅）；保存提交 `roles[]`。仅当角色集为空时提示「至少保留一个角色」。

### 2.25 删除/移除确认文案（#2025 D4）

| 场景 | 文案 |
|------|------|
| 网点移除成员 | 「将移除其 {网点名} 员工身份，账户保留」 |
| 商户移除成员 | 「将移除其 {商户名} 及下属网点全部身份，账户保留」 |
| 系统管理员标记删除 | 「将解除全部关联并标记删除，账户可重新加回」 |
...
**位置**: 人员管理页 →「创建用户」（原 Modal 内「搜索用户/创建用户」双 Tab 已移除——搜索 Tab 为死 Tab，state 从未写入）

- **表单首项 = 用户类型**（Radio）：网点员工 / 维修师傅 / 商户直属员工
- 条件字段：
  - 网点员工：归属网点（必填）+ 角色
...
- **#2073：照片 + 富文本简介对「所有类型」开放**（原先仅维修师傅）：照片=**头像**（创建后经 `POST /api/admin/users/:id/avatar` 上传，256²WebP）；简介富文本落 `users.bio`（师傅另双写 `technician_profiles.bio`）；`technician_profiles.photo`（师傅列表/档案展示）仍由「维修师档案」页管理
- **#2070 细化**：
...
### 3.21 乐币规则（`/system/gift-policies`，菜单原「赠点策略」）（#1939 / #1945）
