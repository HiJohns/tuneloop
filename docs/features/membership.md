...
> ⚠️ **已废弃（#1899 方案 A）**：返点配置页/接口已移除，实际返点由 `gift_policies.refund_ratio` 承担；本节的 `rebate_config` 仅存档参考。
...
> ⚠️ **已废弃（#1899 方案 A）**：`discount_policy` 类折扣已移除入口；会员折扣政策见 #1543。
