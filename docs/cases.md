...
| type | 说明 | id 含义 |
|------|------|---------|
| `rent` | 租赁订单支付 | order_id |
| `repair` | 维修报价支付 | repair_request_id |
| `requote` | 维修重新报价支付 | repair_request_id |
| `damage` | 定损支付 | order_id |
| `points` | 点数购买 | (待定) |
| `refund` | 退款 | order_id |
| `deposit-refund` | 押金退款 | order_id |
...
4. **支付执行**
   ...
   - 现金差额 ≤ 0：显示"确认支付 ¥0（使用点数）"按钮
   - 调用 `POST /api/pay/prepay { order_id, order_type, amount }`
   - mock 模式：直接显示"支付成功"并跳转
   - 生产模式：返回 prepay 参数，调起微信支付 JSAPI

5. **进入入口**
   - **结算页**：batchCreate 成功后自动跳转 → `/payment?type=rent&id={order_id}`
   - **我的租赁列表**：'立即支付'按钮 → 直接跳转支付页（不经过订单详情）
   - **订单详情（weapp）**：'去支付'按钮 → 跳转支付页
   - **维修报价页**：'确认支付'按钮 → `/payment?type=repair&id={requestId}`
