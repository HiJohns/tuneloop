...
4. **收货地址**
   - 批量下单流程（购物车）收集单一收货地址，单件下单在结算页独立填写
...
2. **一阶段支付（当前）**
   - 点击支付 → 生成订单记录（状态标记为 `paid`，模拟支付成功）→ 清空购物车对应项 → 跳转完成页
   - 跳过真实支付网关，但订单直接进入已支付状态
   - **库存软锁**：创建订单时标记 instrument 为 `rented`
...
报修指用户将自有乐器发往指定网点维修。流程涉及 11 个状态、4 个角色。
...
**用户**：在移动端维修页可查看自己的报修单，创建新报修单（填写类型/品牌/型号/识别码/描述/照片，选择商户→网点）。
- 识别码输入有 500ms 防抖，自动查找已有自有乐器记录并填充
- 自有乐器 SN+类型+品牌+型号 组合确定唯一性
- 有物流单号 → 发送中；无 → 待发送（可事后补填）
- 待回复 → 确认(→待付款→支付→维修中) 或 拒绝(→待取消→支付→待发回)
- 支付取整：现金/运费 Math.ceil，点数 Math.floor
- 已发回 → 评价 或 申诉

**员工**：收货时扫描识别码，智能匹配报修单（→质检中）vs 租赁乐器（→接收定损）。匹配到多条记录时弹窗选择。
- 待发回报修单 → 填写发回物流 → 已发回

**维修师傅**：查看质检中和维修中报修单 → 质检中(拍照+报价→待回复) / 维修中(拍照+评论+完成→待发回)
- 强制要求拍摄识别码特写照片

**商户管理员**：PC 端查看商户下属各网点的报修列表，按状态/网点筛选。
...
```
顾客创建报修单 → 选择"合作商家"→ 显示中转网点列表
       → 选中中转网点 → 创建报修中转单 (inbound)
中转网点 → 收货拆包拍照
       → 选择关联受控网点转发 → 拆包拍照重新打包
       → 系统自动创建真实报修单（报修人=中转网点+中转单号）
       → 状态→流入
受控商户 → 质检报价 → 维修 → 完成
```
...
```
受控商户 → 打包发往中转网点（留言填中转单号）→ 流出
中转网点 → 收货拆包拍照 → 转包发往顾客 → sent_back
顾客端 → 收到已发回通知 → 评价/申诉
```
