...
2. 点击"批量删除"按钮
3. 确认弹窗显示删除数量
4. 系统逐个校验删除条件，返回部分成功结果：
   - 全部成功：提示"已成功删除 N 个乐器"
   - 部分失败：弹窗显示"成功删除 X 个，N 个失败"，列出每个失败项及原因

#### 失败原因

| 原因 | 显示 |
|------|------|
| 乐器正在使用中 | 乐器正在使用中，无法删除 |
| 乐器存在关联订单 | 乐器存在关联订单（历史交易），无法删除 |
| 乐器不存在 | 乐器不存在或已被删除 |
...
3. **统一支付页（Payment）**
   - 调用 `POST /api/pay/calculate { type: "rent", id }` 获取支付详情
    ...
    - 支持点数抵扣：赠点（赠点上限 = min(余额, floor(应付×**用户当前级别 pay_ratio**))，比例由乐币规则配置）
    - 现金差额 = 应付总额 - 赠点使用
...
#### 订单详情费用明细展示（#1800 单一口径）

> 订单详情「费用信息」由后端 `fee_detail` 三段结构（`paid_block` / `payable_block` / `net_block`）渲染，**单一数据源** `computeSettlement`：returning 态实时计算，settled 态从落库 `settlements.breakdown` 重建现场（两态数字一致）。金额全部分，前端 `/100` 显示。
...
续费 ¥X.XX（续费日期 M月D日）
    第2阶梯 ¥R/天 × D3天 = ¥S3
    ……（其他续费类似；历史记录无 days 时仅金额，无阶梯）
————————
合计实付 ¥X.XX（= 合同租金 + 押金 + Σ续费）
```
...
**阶梯编号全局连续**：首期阶梯从第 1 阶梯起；续费阶梯延续首期后的编号（如首期占满第 1 阶梯，续费从第 2 阶梯起）。阶梯明细只取决于下单时定价策略 + 基准价（首日日租），续费沿用，不按续费时点重新定价。
...
> **⚠️ 已废弃（#1888 R6）**：本节描述的独立维修师傅账户体系（maintenance_workers）已被 `site_members.role='repair_technician'` 取代；师傅的创建/移除经**网点管理 → 成员管理**完成。遗留模块（tickets/workers/sessions）由 #1886 清理。
...
记录列表展示（#1873，时间格式按 ui.md §1.6）：
- 每条记录分行显示：评论正文、照片缩略图（可点击预览原图）、`时间 · 提交人` 元信息行
- 提交者由后端按 `worker_id`（IAM sub）解析展示名，无匹配时省略
- 历史假文件名记录（mock 时代）不渲染为图片
...
- 点击"是"则当前用户成为新负责人
- 点击"否"则退出
- **R4**：仅**同站点**师傅可接手（操作员站点 ∩ 乐器 `current_site_id`）；跨站点接手被拒
...
- **R1（禁止自验收）**：验收人须为**乐器所在站点**的 `site_admin/site_member`，且 **≠ 本单维修负责人**（`repair_worker_id`），兼职身份也不例外
- **R3**：验收不通过的原因写入维修记录（`repair_records`，comment 前缀「验收驳回：」），记录列表可见
...
- 维修中乐器（其他人负责）→ 提示当前负责人
- 已修复乐器 → 显示"验收通过"按钮（**本网点员工可见**，且非本单维修人，见 R1）

> **流程规则补充（#1888）**：
> - **R2**：定损/收货判定损坏时写入 `current_site_id` = 操作员站点（验收站点归属的前提）
> - **R5**：改派负责人（`reassign`）仅 `site_admin`
> - 遗留维保工单（maintenance_tickets）模块**已废弃**（R6），本流程不依赖

### 3.3.7 验收驳回记录（R3）
- 员工点击"验收不通过" → 必填原因（weapp 弹窗输入 / H5 prompt）
- 系统写入一条 `repair_records`：`worker_id = 验收人`，`comment = "验收驳回：" + 原因`
- 乐器 `repair_status` 回退 `repair_in_progress`；原负责人可继续处理
...
> 📌 **详细设计**（会话状态与段映射 / 物流费分段与承担矩阵 / 中转网点配置字段 / 中转工作台交互）：见 `docs/cases/transit.md`。
...
- 会员中心显示当前等级（不再是"普通会员"）
- `GET /user/points` 返回当前级别 `pay_ratio`（乐币规则 pay_ratio，供前端展示抵扣上限；旧 PointsPolicy.MaxPayRatio 默认 0.3 已并入乐币规则）
...
网点员工"接收"（returning）
  ...
  │      → 差额结算退款（含物流费扣除、逾期费、追加物流费、赠点分账，见「退款差额结算」）
...
- **乐币抵扣**：`A1 = floor(应付租金 × 当前级别 pay_ratio)` 与实付 `A0` 取小
- **现金应付**：`C1 = 应付合计 - A1`（无押金参与时）；押金参与时另行扣减
- **退回乐币**：`A0 - A1`（A1 < A0 时），退回原批次（保留原到期日）
- **退回微信**：`C0 - C1`，走微信原路退款
- **实际退款合计**：`已收 - 应付合计`（最小 0）= 退回乐币 + 退回微信
- **不再发放退款返点**（#1945 取消"退款返点给自己"，`refund_ratio` 字段废弃）

### 关键规则

| 规则 | 说明 |
|------|------|
| 物流费支付 | **归还时填写物流单号，但费用计入结算**——不单独收费，作为结算明细的独立行（`shipping_fee`）；**押金仅扣逾期/定损，物流费不再从押金里统一扣**（#1784） |
...
| 退款通知 | 所有退款通知均包含标准收据明细（含乐币分账行 + 会员中心链接） |
| 关单与累计 | 退款执行后订单 `completed`（已完成/done），`total_spending` 按 **C1**（实付现金）累计 |
| 结算预览页 | ReturnSettlement 显示"等待定损，非最终费用"醒目提示 |

### 退款差额结算（乐币规则）

> 统一定义：付款时 `R0 = C0 + A0`（现金 + 乐币抵扣）；退款时按调整后应付 `R1` 重算。

#### 付款侧（初次付款 + 续费通用）
- 乐币抵扣上限：`A0 ≤ floor(应付总额 × 用户当前级别 pay_ratio)`
- 比例来源：**乐币规则**（gift_policies 表，namespace_admin 在 PC「系统管理 → 乐币规则」按会员级别配置）
...
- 重算乐币上限：`A1 = floor(R1 × 当前级别 pay_ratio)`
- 分账规则：
  - `A1 < A0`：退 `A0−A1` 回原批次（保留原到期日）；退 `C0−C1` 回微信（`C1 = R1 − A1`）
  - `A1 ≥ A0`：乐币不退（仍为 A0）；退现金 `C0 − (R1 − A0)`
  - 守恒校验：`退乐币 + 退现金 = R0 − R1`
- 支付回调若用了乐币，必须同步扣减 `cash_paid`（防结算双计）

#### 退款后动作
1. 订单状态 → `completed`（已完成/done）
2. `total_spending += C1`（**实付现金口径**——行业惯例按实付计成长值，防乐币循环放大）
3. **不再发放退款返点**（#1945 取消"退款返点给自己"）
4. 完成通知：完整收据 + 感谢语 + 乐币退回 + 会员中心链接

#### 累计花销口径决策（R1 vs C1）
**采用 C1（实付现金）**：
- 行业惯例：航司里程/信用卡积分/电商成长值均按实付金额累计
- 防循环：若按 R1（含乐币面值），「用乐币→累计消费→升级→返更多乐币」形成复利放大
