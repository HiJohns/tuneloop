      ...
      <div className="absolute top-0 right-0 flex">
        ...
        {isFailed ? (
          ...
          <Button
            size="small"
            icon={<DeleteOutlined />}
            onClick={() => onRemove && onRemove(file.uid)}
            danger
            disabled={!onRemove}
          />
        )}
  ...
  const fetchInstrument = async () => {
    ...
    try {
      ...
      if (data.code === 20000) {
        ...
        form.setFieldsValue(instrumentData)
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (result.code === 20000 || result.code === 20100) {
        message.success(id && id !== 'new' ? '更新成功' : '创建成功')
    ...
    <div className="p-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        >
          ...
          <Form.Item label="图片" extra="拖拽可调整图片顺序，建议尺寸 800x600">
            <div>
              <Upload
          ...
          <Form.Item name="video" label="视频URL">
        ...
        <div className="mt-6 flex justify-end gap-3">
          <Button onClick={() => navigate('/instruments/list')}>取消</Button>
          <Button type="primary" loading={loading || uploadStatus.isUploading} onClick={handleSubmit}>
            保存
          </Button>
        </div>
      </div>
